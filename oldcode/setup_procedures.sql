-- =============================================================================
-- SIMPLIFIED PROCEDURES FOR LOGISTICS SYSTEM
-- Skip triggers with DELIMITER issues, focus on core functionality
-- =============================================================================
...
BEGIN
    IF p_FreightCost <= 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lỗi: Chi phí vận chuyển phải lớn hơn 0!';
    END IF;

    IF p_PickupLocation IS NULL OR TRIM(p_PickupLocation) = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lỗi: Điểm lấy hàng không được để trống!';
    END IF;

    IF p_DeliveryLocation IS NULL OR TRIM(p_DeliveryLocation) = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lỗi: Điểm giao hàng không được để trống!';
    END IF;
...
BEGIN
    ...
    IF v_ItemPrice IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lỗi: Mặt hàng không tồn tại!';
    END IF;
...
SELECT 'Simplified procedures created successfully!' AS Status;
